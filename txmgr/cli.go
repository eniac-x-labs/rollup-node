...
var (
	DefaultDappRollupFlagValues = DefaultFlagValues{
...
func CLIFlags(envPrefix string) []cli.Flag {
	return CLIFlagsWithDefaults(envPrefix, DefaultDappRollupFlagValues)
}

func CLIFlagsWithDefaults(envPrefix string, defaults DefaultFlagValues) []cli.Flag {
	prefixEnvVars := func(name string) []string {
		return service.PrefixEnvVar(envPrefix, name)
	}
	return append([]cli.Flag{
		&cli.StringFlag{
			Name:     L1RPCFlagName,
			Usage:    "The rpc of l1 client.",
			Required: true,
	...
	}, signer.CLIFlags(envPrefix)...)
}

type CLIConfig struct {
	L1RPCURL                  string
	PrivateKey                string
	SignerCLIConfig           signer.CLIConfig
	NumConfirmations          uint64
	SafeAbortNonceTooLowCount uint64
	FeeLimitMultiplier        uint64
	FeeLimitThresholdGwei     float64
...
func NewCLIConfig(l1RPCURL string, defaults DefaultFlagValues) CLIConfig {
	return CLIConfig{
		...
		SignerCLIConfig:           signer.NewCLIConfig(),
	}
}
...
func ReadCLIConfig(ctx *cli.Context) CLIConfig {
	return CLIConfig{
		...
		SignerCLIConfig:           signer.ReadCLIConfig(ctx),
...
func NewConfig(cfg CLIConfig, l log.Logger) (Config, error) {
	...
	signerFactory, from, err := signer.SignerFactoryFromConfig(l, cfg.PrivateKey, cfg.SignerCLIConfig)
	if err != nil {
