...
// data.
func MakeSidecar(blobs []*eth_serivce.Blob) (*types.BlobTxSidecar, []common.Hash, error) {
	...
	for i, blob := range blobs {
		...
		blobHashes = append(blobHashes, eth_serivce.KZGToVersionedHash(commitment))
	}
...
func (m *SimpleTxManager) queryReceipt(ctx context.Context, txHash common.Hash, sendState *SendState) *types.Receipt {
	...
	m.l.Debug("Transaction mined, checking confirmations", "tx", txHash,
		"block", eth_serivce.ReceiptBlockID(receipt), "tip", eth_serivce.HeaderBlockID(tip),
	...
	if txHeight+m.cfg.NumConfirmations <= tipHeight+1 {
		m.l.Info("Transaction confirmed", "tx", txHash,
			"block", eth_serivce.ReceiptBlockID(receipt),
